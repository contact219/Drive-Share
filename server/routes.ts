...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/social", async (req: Request, res: Response) => {
    try {
      ...
      if (!provider || !token) {
        return res
          .status(400)
          .json({ error: "Provider and token are required" });
      }
      ...
      if (provider === "google") {
        ...
        try {
          ...
          if (parts.length !== 3) {
            return res
              .status(401)
              .json({ error: "Invalid Apple token format" });
          }
          ...
          return res
            .status(401)
            .json({ error: "Failed to verify Apple token" });
        }
      }
      ...
      if (!verifiedEmail) {
        return res
          .status(400)
          .json({ error: "Could not verify email from provider" });
      }
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        const ownerVehicles = await storage.getOwnerVehicles(
          req.params.ownerId,
        );
    ...
    try {
      ...
      const enrichedVerifications = await Promise.all(
        verifications.map(async (v) => {
          ...
          const ownerProfile = v.ownerId
            ? await db_getOwnerProfileById(v.ownerId)
            : null;
          const owner = ownerProfile
            ? await storage.getUser(ownerProfile.userId)
            : null;
          return {
            ...v,
            vehicle,
  ...
  app.get(
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch pending verifications" });
      }
    },
  );
  ...
  app.post("/api/insurance", async (req: Request, res: Response) => {
    try {
      ...
      if (!["platform", "owner"].includes(providerType)) {
        return res
          .status(400)
          .json({ error: "providerType must be 'platform' or 'owner'" });
      }
  ...
  app.post("/api/user/documents", async (req: Request, res: Response) => {
    try {
      ...
      if (!userId || !documentType) {
        return res
          .status(400)
          .json({ error: "Missing required fields: userId, documentType" });
      }

      if (
        !["drivers_license", "insurance_card", "proof_of_identity"].includes(
          documentType,
        )
      ) {
        return res.status(400).json({
          error:
            "Invalid documentType. Must be: drivers_license, insurance_card, or proof_of_identity",
        });
      }
